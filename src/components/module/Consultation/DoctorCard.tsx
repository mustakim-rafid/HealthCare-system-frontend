    ...
    <>
      <Card className="overflow-hidden hover:shadow-lg transition-shadow h-full flex flex-col">
                ...
                {doctor.doctorSpeciality &&
                  doctor.doctorSpeciality.length > 0 && (
                    <Badge variant="secondary" className="text-xs">
                      {doctor.doctorSpeciality[0].speciality?.title}
                    </Badge>
                  )}
              </div>
            </div>
        ...
        <CardContent className="space-y-3 pb-3 flex-1">
          ...
          {doctor.doctorSpeciality && doctor.doctorSpeciality.length > 1 && (
            <div className="flex flex-wrap gap-1">
              {doctor.doctorSpeciality.slice(1, 3).map((eachSpeciality) => (
                <Badge
                  key={eachSpeciality.specialityId}
                  variant="outline"
                  className="text-xs"
                >
                  {eachSpeciality.speciality.title}
                </Badge>
              ))}
              {doctor.doctorSpeciality.length > 3 && (
                <Badge variant="outline" className="text-xs">
                  +{doctor.doctorSpeciality.length - 3} more
                </Badge>
              )}
            </div>
          )}
        ...
        <CardFooter className="border-t flex gap-2 items-stretch">
          <Link
